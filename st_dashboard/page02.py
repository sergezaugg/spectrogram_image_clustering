...
if len(ss['dapar']['X']) > 0 :
    ...
    ss['dapar']['X2D'] = dim_reduction_for_2D_plot(X = ss['dapar']['X'], n_neigh = ss['upar']['umap_n_neighbors'], n_components = 2)
    ss['dapar']['X_dimred'] = dim_reduction_for_clustering(X = ss['dapar']['X'], n_neigh = ss['upar']['umap_n_neighbors'], n_dims_red = ss['upar']['umap_n_dims_red'])
    gc.collect()
    #-------------------------------------------
    ...
    with cols[2]:
        with st.container(border=True, height = 250): 
            st.text("Dimension")  
            st.info(str(ss['dapar']['X_dimred'].shape[0]) + '  imgs')
            st.info(str(ss['dapar']['X_dimred'].shape[1]) + '  feats')
    with cols[3]:
        with st.container(border=True, height = 250): 
