...
with c00:
    with st.container(border=True) :  
        ...
        * MP3 converted to WAVE and resampled to 24'000 samples per second.
        * WAVE sequentially chunked into pieces of 0.4 seconds. 
        * Pieces where transformed to spectrograms of 128 frequency by 256 time bins. 
        * Spectrograms capture acoustic frequencies up to 12 KHz.                        
        * These 25157 spectrograms stored as RGB images. 
        * Smaller random subsample of 10'000 images used in dashboard to avoid memory issues.   

        ### Feature extraction 
        * RGB spectrograms were fed into pre-trained image DNNs.            
        * Arrays returned by internal convolutional layers used for feature extraction.
        * These arrays were converted to linear feature vectors (Details in the right panel).  
        * Feature extraction was pre-computed on a GPU machine.    
        ...
        * Based on recommendations from [Best et al. 2023](https://doi.org/10.1371/journal.pone.0283396), we provide dim reduced data of 4, 8 and 16 dims.                
        ...
        ### Data storage   
        * RGB spectrograms and extracted features stored on a dedicated [Kaggle dataset](https://www.kaggle.com/datasets/sezaugg/spectrogram-clustering-01)           
...
with c01:
    ...
    with st.container(border=True) : 
        ...
        st.markdown(''' 
            ...
            To date, 3 CNNs (DenseNet121, vgg16, ResNet50) and one vision transformer (MaxVit_T) were assessed. 
            ...
            Full pooling over time was used to make final features invariant to time location of patterns.
